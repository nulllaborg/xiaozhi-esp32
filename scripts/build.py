...
def _normalize_language(language: str) -> str:
    ...
    languages_by_casefold = {
        supported.casefold(): supported for supported in supported_languages
    }
...
def _collect_languages(
    ...
    mappings = re.findall(
        r"(?:if|elseif)\(CONFIG_LANGUAGE_([A-Z_]+)\)\s*" r'set\(LANG_DIR "([^"]+)"\)',
        cmake,
    )
    if not mappings:
...
def _collect_wake_words(
    ...
    for index, match in enumerate(config_matches):
        ...
        suffix = f"({model})"
        phrase = label[: -len(suffix)].strip() if label.endswith(suffix) else label
        targets = (
            _WAKE_WORD_TARGETS
...
def _kconfig_choice(
    ...
    matches = list(
        re.finditer(r"^\s*config\s+([A-Za-z0-9_]+)\s*$", block, re.MULTILINE)
    )
...
def _kconfig_config_board_dependencies(
    ...
    remainder = content[start.end() :]
        ...
        r"^\s*(?:config\s+|endchoice\s*$|endmenu\s*$)", remainder, re.MULTILINE
    )
...
def _build_option_definitions(
    ...
    if not isinstance(sdkconfig_append, list) or not all(
        ...
        raise ValueError(
            f"build {build.get('name')!r} sdkconfig_append must be a string list"
        )
    ...
    for choice_name in ("DISPLAY_OLED_TYPE", "DISPLAY_LCD_TYPE"):
        ...
        if board_config not in choice["board_configs"]:
            continue
        entries = [
            entry for entry in choice["entries"] if entry["value"] != "LCD_CUSTOM"
        ]
        definitions.append(
            {
                "key": "display_model",
        ...
        definitions.extend(
            (
                ...
                },
                {
                    "key": "multiline_chat",
                    "type": "boolean",
                    "default": assignments.get("CONFIG_USE_MULTILINE_CHAT_MESSAGE")
                    == "y",
                },
            )
        )
        ...
        definitions.append(
            {
                ...
                "type": "select",
                "default": "device"
                if assignments.get("CONFIG_USE_DEVICE_AEC") == "y"
                else "off",
                "choices": [
                    {"value": "off", "label": "Off"},
    ...
    if target != "esp32p4" and (
        "wifi_board.h" in source or re.search(r"\bWifiBoard\b", source)
    ):
        definitions.append(
            {
    ...
    has_common_camera = ("new Esp32Camera" in source or "new EspVideo" in source) and (
        camera_enable_symbol is None or assignments.get(camera_enable_symbol) == "y"
    )
...
def _build_options_sdkconfig(
    ...
    if "display_model" in options:
        ...
        for choice in by_key["display_model"]["choices"]:
            result.append(
                f"CONFIG_{choice['value']}={'y' if choice['value'] == selected else 'n'}"
            )
    ...
    if "display_style" in options:
        ...
        if (
            ...
            selected != "emote"
            and base_assignments.get("CONFIG_FLASH_EXPRESSION_ASSETS") == "y"
        ):
            result.extend(
    ...
    if "multiline_chat" in options:
        result.append(
            f"CONFIG_USE_MULTILINE_CHAT_MESSAGE={'y' if options['multiline_chat'] else 'n'}"
        )
    ...
    if idf_path:
        ...
        if version_file.exists():
            ...
            for line in version_file.read_text(encoding="utf-8").splitlines():
                match = re.match(
                    r"set\(IDF_VERSION_(MAJOR|MINOR|PATCH)\s+(\d+)\)", line
                )
                if match:
...
def _collect_variants(
    ...
    for variant in variants:
        ...
        build = next(
            item
            for item in _get_builds_for_idf(cfg, idf_version)
...
def _select_variants_for_changes(
    ...
    """
    known_boards = sorted(
        {variant["board"] for variant in variants}, key=len, reverse=True
    )
    ...
    for raw_path in changed_files:
        ...
        if (
            ...
            or path.startswith("partitions/")
            or path.startswith("sdkconfig.defaults")
            or (path.startswith("main/") and not path.startswith("main/boards/"))
            or path.startswith("main/boards/common/")
        ):
            return variants
...
def _extract_board_config_from_sdkconfig_append(
    sdkconfig_append: list[str],
) -> Optional[str]:
    ...
    if len(uniq) > 1:
        raise ValueError(
            f"Multiple board type configs found in sdkconfig_append: {uniq}"
        )
    return uniq[0]
...
def _symbol_supports_target(symbol: str, target: str) -> bool:
    ...
    for line in lines:
        ...
        if stripped.startswith("config "):
            ...
            continue
        if in_symbol and stripped.startswith(
            ("config ", "choice ", "endchoice", "menu ", "endmenu")
        ):
            break
...
def _sync_vscode_target(
    ...
    pattern = re.compile(
        r'(?P<prefix>"IDF_TARGET"\s*:\s*")' r'(?:\\.|[^"\\])*' r'(?P<suffix>")'
    )
...
def build_board(
    ...
    *,
    ...
    builds = _get_builds_for_idf(cfg, idf_version)
    builds = [build for build in builds if _get_reported_name(build) == name_filter]
    if not builds:
        print(
    ...
    for build in builds:
        ...
        explicit_board_cfg = _extract_board_config_from_sdkconfig_append(
        ...
        if explicit_board_cfg == board_type_config:
            ...
            sdkconfig_append.extend(
                item
                for item in build_sdkconfig_append
        ...
        if language is not None:
            ...
            validation_symbols.append(([option.split("=", 1)[0]], "--language"))
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument(
        "--name", help="build.name to compile (the OTA-reported board name)"
    )
    parser.add_argument(
        "--language",
        metavar="LOCALE",
    ...
    if args.select_changed:
        if (
            ...
            parser.error(
                "--select-changed cannot be combined with build or list options"
            )
        idf_version = _detect_idf_version_for_listing()
        variants = _collect_variants(
            config_filename=args.config, idf_version=idf_version
        )
    ...
    if args.list_boards:
        ...
        if (
            ...
            parser.error(
                "--list-boards cannot be combined with build or other " "list options"
            )
        idf_version = _detect_idf_version_for_listing()
        variants = _collect_variants(
            config_filename=args.config, idf_version=idf_version
    ...
    if board_type_input != "all" and not _board_type_exists(board_type_input):
        print(
            f"[ERROR] board_type {board_type_input} not found in main/CMakeLists.txt",
            file=sys.stderr,
        )
        sys.exit(1)

    variants_all = _collect_variants(
        config_filename=args.config, idf_version=idf_version
    )
    ...
    if board_type_input == "all":
        ...
        selected_variants = [
            ...
            if variant["board"] == board_type_input and variant["name"] == name_filter
        ]
    ...
    for variant in selected_variants:
        ...
        if not _board_type_exists(bt):
            print(
                f"[ERROR] board_type {bt} not found in main/CMakeLists.txt",
                file=sys.stderr,
            )
            sys.exit(1)
